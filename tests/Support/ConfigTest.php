...
final class ConfigTest extends TestCase {
	...
	public function test_defaults_match_v1(): void {
		$config = Config::fromArray( 'my-plugin', 'My Plugin' );
		...
		$this->assertSame( 'duckdev', $config->domain() );
		$this->assertSame( '', $config->message() );
		// Slug → prefix conversion preserves the v1 storage layout.
	...
	public function test_overrides_are_applied(): void {
		...
		$this->assertSame( 'Sure', $config->actionLabels()['review'] );
		// Other action labels fall back to defaults.
		$this->assertSame( 'Nope, maybe later', $config->actionLabels()['later'] );
	}

	public function test_classes_always_include_base(): void {
		$config = Config::fromArray( 'my-plugin', 'My Plugin', array( 'classes' => array( 'is-dismissible' ) ) );
